...
app.MapGet("/api/media/browse", async (HttpRequest req, IDbContextFactory<VideoIndexDbContext> factory) =>
{
    ...
    var pre = await qEF.Select(m => new { m.Id, m.Filename, m.Title, m.IsFavourite, m.Path, m.SizeBytes, m.LengthSeconds, m.Width, m.Height, m.Year, m.PerformerCount, m.PerformerNames, m.SourceTypes, m.OrientationTags, m.OtherTags, m.StudioName, m.SourceUsername, RootName = m.Root != null ? m.Root.Name : null, m.UpdatedAt }).ToListAsync();
